...
describe("Main Test Suite", async () => {
  ...
  describe("JomEV Core", async () => {
   it("should add users, add provider and create station", async () => {
      ...
      await JomEV.connect(deployer).addStation(parseUnits("0.000005"),"Damansara",dummyToken.address);
   })
   it("can book station", async ()=> {
      ...
      await JomEV.connect(deployer).bookStation(1,1,"0x101000",dummyToken.address);
   })
   it("booking details are right", async ()=> {
    ...
    await approve(deployer,JomEV.address,dummyToken,parseUnits("0.000005"));
    await JomEV.connect(deployer).bookStation(1,1,input1,dummyToken.address);
    await approve(deployer,JomEV.address,dummyToken,parseUnits("0.000005"));
    await JomEV.connect(deployer).bookStation(1,1,input2,dummyToken.address);
   ...
   it("can desactivate stations", async () => {
    ...
    await JomEV.connect(deployer).joinAsProvider();
    await approve(deployer,JomEV.address,dummyToken,parseUnits("0.000005")*24*8)
    await JomEV.connect(deployer).addStation(parseUnits("0.000005"), "Bangsar South",dummyToken.address);   
    await JomEV.connect(deployer).desactivateStation(1);
    const stationData = await JomEV.getStation(1);
   ...
   it("cannot book station if time overlaps", async ()=>{
    ...
    await approve(account1,JomEV.address,dummyToken,parseUnits("0.000005"));
    await JomEV.connect(account1).bookStation(1,1,"0x188100",dummyToken.address);
    await approve(account1,JomEV.address,dummyToken,parseUnits("0.000005"));
    await expect(JomEV.connect(account1).bookStation(1,1,"0x100000",dummyToken.address)).to.be.revertedWith("new schedule overlaps");
   })
  })
});